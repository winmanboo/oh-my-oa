    ...
    >
      ...
      <el-table-column label="操作" width="250" align="center">
        <template slot-scope="scope">
          ...
          <el-button type="text" v-if="scope.row.status === 0" size="mini" @click="edit(scope.row.id)"
                     :disabled="$hasBP('bnt.processTemplate.templateSet')  === false">修改审批设置
          </el-button>
          <el-button type="text" v-if="scope.row.status === 0" size="mini" @click="removeDataById(scope.row.id)"
